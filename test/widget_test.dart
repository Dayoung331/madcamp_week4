...
class PastAnswersScreen extends StatelessWidget {
  ...
  Future<String> _loadQuestion(int dayOfYear) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? questionsString = prefs.getString('questions');
    if (questionsString != null) {
      List<String> questions = List<String>.from(json.decode(questionsString));
      return questions[(dayOfYear - 1) % 366];
    }
            ...
            children: [
              ListTile(
                ...
                onTap: () async {
                  ...
                  int dayOfYear = int.parse(dateKey);
                  String question = await _loadQuestion(dayOfYear);
                  _showEditBottomSheet(context, year, question, contentController);
                },
              ),
              ListTile(
  ...
  void _showEditBottomSheet(BuildContext context, String year, String question, TextEditingController contentController) {
    showModalBottomSheet(
      context: context,
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.vertical(top: Radius.circular(25.0)),
